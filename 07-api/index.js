...
require('dotenv').config(); // sanity check console.log(process.env.MONGO_URI)
...
async function main() {
    ...
    app.get('/welcome', function (req, res) {
        res.json({
            'message': 'Welcome to the free food sighting'
        })
    })

    // To design an API endpoint
    // 1. What's the URL (RESTFUL)
    // 2. Determing the verb:
    //      get: retrieve info
    //      post: add new info
    ...
    app.get('/free_food_sighting/', async function (req, res) {
        // create criteria object (assumption: the user wants everything)
        let criteria = {}
...
// LISTEN (must be the last part of Express application)
